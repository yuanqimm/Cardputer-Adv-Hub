...
| SSH | 未连接时 C 连接；首次遇到主机时校验指纹后按 T 信任并保存 |
...
连接分阶段执行：Wi-Fi、握手、主机指纹、认证、PTY、shell。网络等待有超时，可用 Fn+Q 取消。首次连接显示服务器 SHA-256 公钥指纹（64 个十六进制字符），请与服务器端实际公钥核对后按 T；信任记录保存至 NVS。已保存的主机指纹不匹配时停止认证，不发送密码。

也可在 `ssh.json` 中配置 `host_key_sha256`，值为事先核验的 64 个十六进制字符指纹；该显式指纹优先于已保存的记录。服务器更换主机密钥后需核验并更新该字段。配置文件应由用户自行保管。

终端为 38 列 × 12 行，支持分段 ANSI 转义、光标定位、清屏/清行、滚屏、CR/LF、退格、Tab、Ctrl+C/D 等、Alt 前缀和方向键。当前为 ASCII 文本与基础 VT100 子集；UTF-8 非 ASCII 字符显示替代符，不承诺完整 Vim/tmux、颜色或所有终端模式。当前支持密码认证，尚未加入私钥认证、设备内 Wi-Fi 编辑器与多主机会话管理。

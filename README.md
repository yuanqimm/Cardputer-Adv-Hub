...
历史配置保存在开发板 NVS，不写回 SD 卡；最多保存 6 条，新增配置达到上限时淘汰最早一条。扫描成功连接过的 Wi-Fi 密码也会保存在 NVS，供下次自动连接。密码仍应按个人设备安全要求管理。当前支持密码认证，尚未加入私钥认证与多主机会话管理。终端为 38 列 × 12 行，支持分段 ANSI 转义、光标定位、清屏/清行、滚屏、CR/LF、退格、Tab、Ctrl+C/D 等、Alt 前缀和方向键；UTF-8 非 ASCII 字符显示替代符。

LibSSH-ESP32 上游示例使用 51200 字节任务栈，本项目相应增加 Arduino 主循环栈；没有 PSRAM，启动 SSH 或媒体前会检查可用堆内存。进入 SSH 时会暂时释放 BLE 协议栈，退出 SSH 后自动恢复 BLE 广播，以给加密握手保留内存。

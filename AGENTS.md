...
- SD Storage 已取代 Media Player：文件页面在 SdStorage.cpp，读写服务在 FileManager.cpp；任意目录选文件播放，不恢复旧常驻媒体索引。参考来源见 docs/sd-storage-reference.md；维护文件写入时运行 tests/native/test_file_manager.cpp。
